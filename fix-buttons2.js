...
for (const file of files) {
  ...
  // Find clearData button
  content = content.replace(
    /<button class="btn-secondary" style="[^"]+" onclick="clearData\(\)">LÀM MỚI<\/button>/g,
  ...
  content = content.replace(
    /style="padding: 15px 30px; font-size: 15px; font-weight: 600; background-color: #e74c3c; color: white; border: none; border-radius: 6px; cursor: pointer;"([^>]*TÍNH)/g,
    'style="height: 48px; padding: 0 30px; font-size: 15px; font-weight: 600; background-color: #e74c3c; color: white; border: none; border-radius: 6px; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; box-sizing: border-box;"$1'
  );
